...
class ReplayMemory(object):
    ...
    def add_experience(self, state, action, reward, new_state, done, preprocessor):
        
        if self.st is not None:
            
            if self.st.shape[0]>=self.capacity:
                self.st = torch.cat((self.st[-self.capacity+1:], preprocessor.process(state)), dim=0)
